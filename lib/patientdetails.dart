              ...
              child: Column(
                ...
                children: <Widget>[
                  ListView.builder(
                      shrinkWrap: true,
                      itemCount:
                          data["telecom"] == null ? 0 : data["telecom"].length,
                  ...
                  ListView.builder(
                      shrinkWrap: true,
                      itemCount:
                          data["address"] == null ? 0 : data["address"].length,
                      itemBuilder: (BuildContext content, int index) {
                        return ListTile(
...
class _DiagnosticData extends State<DiagnosticData> {
  ...
  Future<List<dynamic>> getData() async {
    ...
    var response = await http.get(
        Uri.encodeFull("http://192.168.1.10:8183/STU3/DiagnosticReport?_id=" +
            data["id"] +
            "&identifier=urn:foo|" +
            token.toString()),
...
class _ClinicalData extends State<ClinicalData> {
  ...
  Future<List<dynamic>> getData2() async {
    dynamic token = await FlutterSession().get("token");
    var response = await http.get(
        Uri.encodeFull("http://192.168.1.10:8183/STU3/Condition?_id=" +
            data["id"] +
            "&identifier=urn:foo|" +
            token.toString()),
        headers: {"Accept": "application/json"});
...
class _MedicationList extends State<MedicationList> {
  ...
  Future<List<dynamic>> getData() async {
    dynamic token = await FlutterSession().get("token");
    var response = await http.get(
        Uri.encodeFull("http://192.168.1.10:8183/STU3/Medication?_id=" +
            data["id"] +
            "&form=urn:foo|" +
            token.toString()),
        headers: {"Accept": "application/json"});
